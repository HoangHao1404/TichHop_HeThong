...
// }
// ======================================================
//  TRANSFORMER.JS - FULL ETL TRANSFORM (PASS1 → PASS2)
//  Clean, generic, dùng TransformEngine + Validation
//  Ghi đầy đủ transform.log, transform_errors.log,
//  Ghi DataQuality_*, không crash, không silent.
// ======================================================
...
// ======================================================
// 4. LOOP TỪNG BẢNG: PASS1 → PASS2 → GHI DATAQUALITY
// ======================================================
for (const table of tables) {
  console.log(`\n==============================`);
  console.log(`${table}`);
  ...
  for (const e of errorRows) {
    ...
    const { newRow, changes } = transformer.apply(before);

    const result = validator2.validateRow(newRow);
  ...
  console.log(`Đã sửa: ${cleaned.length}`);
  console.log(`Không sửa được: ${stillErrorRows.length}`);

  // ===== Ghi transform_errors.csv =====
  if (stillErrorRows.length > 0) {
    for (const err of stillErrorRows) {
      const line = `${err.table},${err.rowNumber},${err.recordID},"${err.errors.join("; ")}"\n`;
      fs.appendFileSync("./validation/logs/transform_errors.csv", line, "utf8");
    }
  }
  // ===== Gộp lại toàn bộ dòng sạch =====
  const allCleanRows = [...validRows, ...cleaned];
  // ===== RESET DataQuality table =====
await db.run(`DROP TABLE IF EXISTS ${dataQualityTable}`);

await db.run(`
  CREATE TABLE ${dataQualityTable} AS
  SELECT * FROM ${stagingTable} WHERE 1=0
`);
  ...
  console.log(`📦 Ghi vào DataQuality: ${allCleanRows.length} dòng sạch`);
}
// 5 Đóng database
await db.close();
console.log(`\n🎉 TRANSFORM HOÀN TẤT!`);
